...
void TestExample::make_circle_waypoints(int direction_, double radious_)
{
  ...
  if(direction_ > 0)
  {
    for(int num = 1; num <= 3; num ++)
    {
      ...
      tf_small_pulley_to_circle_points_ = Transform3D<> (Vector3D<>(x_, y_, -0.02), RPY<>(rotation_z_,rotation_y_,rotation_x_).toRotation3D()); // RPY
  ...
  else
  {
    ...
    for(int num = 1; num <= 3; num ++)
    {
      ...
      tf_big_pulley_to_circle_points_ = Transform3D<> (Vector3D<>(x_, y_, -0.005), RPY<>(rotation_z_,rotation_y_,rotation_x_).toRotation3D()); // RPY
